...
% Indeks Bias Menggunakan Tabel Eksperimen
...
clc
% Input Material
matlu = 'Silica.dat'; % Material Luarold
matdal = 'silverpalik.dat'; % Indeks Bias Material Dalam
nling = 1.3334; % Indeks Bias Lingkungan
erei = 300:1:700; % Input Panjang Gelombang
rdal = 20; % jari - jari dalam
rlua = 25; % jari - jari luar
l = 50; % banyak iterasi
...
[enj, nj, kj] = textread(matdal, '%f %f %f','commentstyle','matlab');

enej = eV2nm ./ enj;
njo = spline( enej, nj, erei );
kjo = spline( enej, kj, erei );
nmatj = njo + i*kjo; % Indeks Bias Material Dalam
...
for j = 1:o
   for t = 1:l
      ...
      Anum = m2(j)*psi*psot - m1(j)*psit*pso;
      Adenum = m2(j)*chi*psot - m1(j)*chit*pso;
      Ane = Anum/Adenum;
      
      Bnum = m2(j)*pso*psit - m1(j)*psi*psot;
      Bdenum = m2(j)*chit*pso - m1(j)*psot*chi;
      Bne = Bnum/Bdenum;
